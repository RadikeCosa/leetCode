...
## Ejemplos paso a paso (en lenguaje natural, muy concreto)

A continuación mostramos cómo leer cada ejemplo como una secuencia de estados (Gray neighbors) y cómo el algoritmo (xor acumulado) lo reproduce.

Ejemplo 1 — n = 3 (binario `11`)

- Secuencia Gray relevante (2 bits): 00 → 01 → 11
  - La posición de `11` en la secuencia Gray es 2 (contando desde 0).
  - Por tanto se necesitan 2 pasos para ir entre 0 y ese estado en la secuencia Gray.

Lectura paso a paso (del estado n hacia 0):

1. Estado inicial: `11` (n = 3).
2. Un paso: `11` → `01` (se invierte el bit que difiere entre ambos estados).
3. Segundo paso: `01` → `00` (se invierte el bit restante).
4. Total de pasos = 2.

El algoritmo xor-acumulado lo calcula así:

- result = 0 ^ 3 = 3, n >>= 1 → n = 1
- result = 3 ^ 1 = 2, n >>= 1 → n = 0
- devuelve 2.

Ejemplo 2 — n = 6 (binario `110`)

- Secuencia Gray relevante (3 bits): 000 → 001 → 011 → 010 → 110
  - La posición de `110` en la secuencia Gray es 4.
  - Por tanto se necesitan 4 pasos para ir entre 0 y ese estado.

Lectura paso a paso (del estado n hacia 0):

1. Estado inicial: `110` (n = 6).
2. Paso 1: `110` → `010`
3. Paso 2: `010` → `011`
4. Paso 3: `011` → `001`
5. Paso 4: `001` → `000`
...
> Observación práctica: en la lectura en términos de "qué índice i elegir en cada paso" puedes pensar: en cada transición Gray solo cambia un bit; elegir el i correspondiente a ese bit (el que difiere) reproduce esa transición. El algoritmo no simula explícitamente índices i, sino que computa la posición en la secuencia Gray (equivalente al número mínimo de transiciones).
