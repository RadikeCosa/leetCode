...
describe("Filter Elements from Array", () => {
  ...
  it("should filter by index position", () => {
    const arr = [1, 2, 3];
    const fn = function firstIndex(n: number, i: number): boolean {
      return i === 0;
    };
    const result = filter(arr, fn);
  ...
  it("should filter out falsy values", () => {
    const arr = [-2, -1, 0, 1, 2];
    const fn = function plusOne(n: number): number {
      return n + 1;
    };
    const result = filter(arr, fn);
  ...
  it("should handle empty array", () => {
    const arr: number[] = [];
    const fn = function alwaysTrue(): boolean {
      return true;
    };
    const result = filter(arr, fn);
  ...
  it("should handle all elements being filtered out", () => {
    const arr = [1, 2, 3, 4, 5];
    const fn = function alwaysFalse(): boolean {
      return false;
    };
    const result = filter(arr, fn);
    expect(result).toEqual([]);
  ...
  it("should handle even numbers filter", () => {
    ...
    const fn = function isEven(n: number): boolean {
      return n % 2 === 0;
    };
    const result = filter(arr, fn);
  ...
  it("should handle truthy and falsy values correctly", () => {
    const arr = [0, 1, -1, 5];
    const fn = function identity(n: number): number {
      return n;
    };
    const result = filter(arr, fn);
  ...
  it("should handle negative numbers", () => {
    ...
    const fn = function positive(n: number): boolean {
      return n > 0;
    };
    const result = filter(arr, fn);
  ...
  it("should handle index-based filtering with multiple conditions", () => {
    ...
    const fn = function evenIndexAndGreaterThan25(
      n: number,
      i: number
    ): boolean {
      return i % 2 === 0 && n > 25;
