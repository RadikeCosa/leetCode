...
export function sortColors(nums: number[]): void {
  ...
  if (debug) console.log("Estado inicial:", [...nums]);

  while (currentIndex <= blueIndex) {
    if (debug) {
      console.log(`\nEvaluando nums[${currentIndex}] = ${nums[currentIndex]}`);
      console.log(
        `redIndex: ${redIndex}, currentIndex: ${currentIndex}, blueIndex: ${blueIndex}`
      );
    }
    ...
    if (nums[currentIndex] === 0) {
      ...
      if (debug) console.log(`→ Es blanco (1), solo avanzar`);
      currentIndex++;
    }

    if (debug) console.log("Estado actual:", [...nums]);
  }
