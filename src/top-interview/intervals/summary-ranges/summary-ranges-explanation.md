...
### Estrategia: Two-Pointer con Single Pass

```typescript
while (start <= end) {
  let rangeStart = nums[start];     // Inicio del rango actual
  let rangeEnd = rangeStart;        // Inicialmente, fin = inicio
  
  // Extender el rango mientras sean consecutivos
  while (start < end && nums[start] + 1 === nums[start + 1]) {
    start++;
  }
  rangeEnd = nums[start];           // Actualizar fin del rango
  ...
  start++;                          // Avanzar al siguiente segmento
}
```
...
```
Iteración 1:
- start=0, rangeStart=0, rangeEnd=0
- nums[0]+1=1 === nums[1]=1 ✓ → start=1
- nums[1]+1=2 === nums[2]=2 ✓ → start=2  
- nums[2]+1=3 ≠ nums[3]=4 ✗ → rangeEnd=2
- resultado: "0->2", start=3

Iteración 2:
- start=3, rangeStart=4, rangeEnd=4
- nums[3]+1=5 === nums[4]=5 ✓ → start=4
- nums[4]+1=6 ≠ nums[5]=7 ✗ → rangeEnd=5  
- resultado: "4->5", start=5

Iteración 3:
- start=5, rangeStart=7, rangeEnd=7
- start=5, end=5 → no hay nums[6]
- resultado: "7", start=6
...
- **Aplicación**: `start` para recorrer, detección de rangos
- **Ventaja**: Una sola pasada, sin backtracking

### 2. **Consecutive Detection Pattern**
```typescript
nums[i] + 1 === nums[i + 1]  // Detecta números consecutivos
```

### 3. **Range Consolidation**
- **Inicio**: Marcar `rangeStart`
...
### Boundary Conditions:
- **`start < end`**: Previene acceso fuera de rango
- **`start <= end`**: Permite procesar último elemento
