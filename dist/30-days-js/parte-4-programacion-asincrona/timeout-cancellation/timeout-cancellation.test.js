...
describe("Timeout Cancellation", () => {
    ...
    const runCancellableTest = async (fn, args, t, cancelTimeMs) => {
        const results = [];
        ...
        await new Promise((resolve) => setTimeout(resolve, Math.max(t, cancelTimeMs) + 50));
        return results;
    };
    ...
    it("should handle functions with different return types", async () => {
        const stringFn = (...args) => args[0].toUpperCase();
        const args = ["hello"];
        const t = 25;
