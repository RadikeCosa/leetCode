 ...
 * Output: [[2,2,3], [7]]
 */
export function combinationSum(candidates, target, debug = false) {
    const result = [];
    candidates.sort((a, b) => a - b);
    function backtrack(start, current, remaining) {
        if (debug) {
            console.log(`🔁 backtrack(start=${start}, current=[${current.join(",")}], remaining=${remaining})`);
        }
        if (remaining === 0) {
            if (debug)
                console.log(`✅ combinación válida: [${current.join(",")}]\n`);
            result.push([...current]);
            return;
        }
        if (remaining < 0) {
            if (debug)
                console.log(`⛔ combinación inválida (se pasó): [${current.join(",")}]\n`);
            return;
        }
        for (let i = start; i < candidates.length; i++) {
            const candidate = candidates[i];
            if (debug)
                console.log(`➡️  pruebo agregar ${candidate}`);
            current.push(candidate);
            backtrack(i, current, remaining - candidate);
            if (debug)
                console.log(`⬅️  saco ${candidate} (backtrack)`);
            current.pop();
        }
    }
    backtrack(0, [], target);
